...
MineSync is a serverless, peer-to-peer approach to hosting a Minecraft server among friends. Instead of paying for a 24/7 dedicated server to host your world, the world directory is stored securely on an S3-compatible cloud storage bucket (like Cloudflare R2).
...
### 1. Configuration
A dummy `.env` file has been provided in the root directory. You must fill this out with your exact S3 remote credentials. 
...
1. Place your desired Minecraft `server.jar` into the same directory as the script.
2. (Optional but recommended) Start the `.jar` manually once without the script to accept the EULA and generate base files.
...
* **Back up the world periodically:** While Cloud storage provides intense durability, manually pushing your `world/` folder into a ZIP on your laptop once a week remains best-practice.
